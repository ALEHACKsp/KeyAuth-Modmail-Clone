...
async function createNewThreadForUser(user, opts = {}) {
  return _addToThreadCreationQueue(async () => {
	...
	const isPaid = userGuildData.size === 0 || Array.from(userGuildData.values()).some(({guild, member}) => {
        return member.roles.includes('824406803827392523') || member.roles.includes('824406788581490708') || member.roles.includes('949428085852356638');
    });
	
	if (isPaid) {
		channelName = `${cleanName}-${user.discriminator}-paid`;
	}
